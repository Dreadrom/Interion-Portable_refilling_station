...
public class GetCpuTemperature extends BaseHTTPRequest<Integer> {
    public static final String REQUEST_NAME = "GetCpuTemperature";
    public static final String RESPONSE_NAME = "CpuTemperature";
    ...
    public boolean parseJSON(JSONObject jsonObject) throws TTException {
        try {
            ...
            if (!dataJSONObject.has("Temperature")) {
                throw new TTException("Error: response doesn't contain Temperature property", Result.PROTOCOL_ERROR);
            }

            int temperature = dataJSONObject.getInt("Temperature");
