...
public class CurrencySelectedState extends BaseState {
	public static final String NAME = "CurrencySelectedState";

	public CurrencySelectedState() {
		super(NAME);
	}
	...
	public boolean onStart(BaseState stateFrom, StateData stateData) {
		...
		return setCurrencyWithStateData(stateData);
	}
	...
	public boolean setCurrencyWithStateData(StateData stateData) {
		...
		AtomicReference<BigDecimal> currency = new AtomicReference<>(new BigDecimal(0));

		CompletableFuture<Boolean> future = CompletableFuture.supplyAsync(() ->
			monitorHelper.parseCurrency(sMonitorText, (result, sError, parsedValue) -> {
					if (!result) {
						viewModel.sendViewModelCommandEvent(showError.toString(), sError);
					}
					else {
						currency.set(parsedValue);
					}
				}
			));
