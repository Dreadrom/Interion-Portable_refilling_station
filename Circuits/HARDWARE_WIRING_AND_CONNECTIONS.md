...
1. **2 IP cameras** — nozzle tamper, licence-plate. Each camera streams ≥1080p30 h.264 to local storage + AWS S3.
...
|---|---|---|---|
| 2 IP cameras | ✅ 2 native ports | ✅ 2 native ports | ✅ 4 ports natively |
...
**Why M2 over M1S:**
- 3 USB ports natively support 2 cameras — no hub required (one port free for WiFi dongle or RS485)
...
- No built-in WiFi → use on-site Ethernet (preferred for stability anyway) OR buy S$13 WiFi USB dongle, consuming one USB port (still leaving 2 for cameras — no hub needed)
...
- Powered USB 3.0 hub is optional for 2 cameras (M2 has 3 native USB ports)
...
### Camera Integration (2 cameras)

| Camera | Use | Interface | Resolution | Notes |
|---|---|---|---|---|
| Nozzle tamper cam (`/dev/video0`) | Detect nozzle inserted/removed, drip detection | USB 3.0 | 1080p30 | Mount at knee height pointing at nozzle |
| Licence plate cam (`/dev/video1`) | Vehicle plate at entry | USB 3.0 | 1080p30 or 4K | Narrow FOV, mount at bumper height |
...
- Run 2 streams simultaneously at ~20 fps each, or round-robin
